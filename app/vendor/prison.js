// this is the lib/highlight.js file from Mattis' prison lib:
// https://github.com/dighl/prison/blob/master/lib/highlight.js
// it is wrapped in a iife in order not to pollute the global namespace
// 
...
var prison = (function() {
	...
	function getSoundClass(sound){
		if(sound in DOLGO){ return DOLGO[sound]; }
		if(sound.slice(0,2) in DOLGO){ return DOLGO[sound.slice(0,2)]; }
		if(sound.slice(0,1) in DOLGO){ return DOLGO[sound.slice(0,1)]; }
		if(sound.slice(1,3) in DOLGO){ return DOLGO[sound.slice(1,3)]; }
		if(sound.slice(1,2) in DOLGO){ return DOLGO[sound.slice(1,2)]; }
		if(sound === "-"){return "-";}
		return 'dolgo_ERROR';
	}
	...
	return {
		...
		'privates': privates,
		'DOLGO': DOLGO,
		'getSoundClass': getSoundClass,
		'plotWord': plotWord,
		'plotMorphemes': plotMorphemes,
		'ipa2tokens': ipa2tokens,
		'_ART': _ART,
		'ART': ART,
		'prosodic_string': prosodic_string
	};
